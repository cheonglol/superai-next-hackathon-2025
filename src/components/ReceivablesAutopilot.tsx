...
const ReceivablesAutopilot: React.FC<ReceivablesAutopilotProps> = ({ mockFinancialData }) => {
  ...
  const needsEscalation = (invoice: any) => {
    return (
      (invoice.status === "overdue" && invoice.daysLate && invoice.daysLate >= 15) || (invoice.remindersSent >= 3 && !invoice.responseReceived) || invoice.status === "collections"
    );
  };
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="text-xs text-gray-700">
                      {invoice.remindersSent === 0
                        ? "No reminders sent"
                        : `${invoice.remindersSent ?? 0} reminder${(invoice.remindersSent ?? 0) > 1 ? "s" : ""} sent${invoice.responseReceived ? ", response received" : ", no response"}`}
                    </div>
                  </td>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div
                    className={`px-2 py-1 rounded-full text-xs font-medium ${
                      customer.score > 70 ? "bg-green-100 text-green-800" : customer.score > 50 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"
                    }`}
                  >
