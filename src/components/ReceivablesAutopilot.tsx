...
import { Zap, TrendingUp, AlertTriangle, CheckCircle, DollarSign, Target, RefreshCw, Clock, Search, Filter, Flag } from "lucide-react";
...
const ReceivablesAutopilot: React.FC<ReceivablesAutopilotProps> = ({ mockFinancialData }) => {
  ...
  const [messageType] = useState<"friendly" | "formal" | "urgent">("friendly");
  ...
  const needsEscalation = (invoice: { status: string; daysLate?: number; remindersSent?: number; responseReceived?: boolean }) => {
    return (
      (invoice.status === "overdue" && invoice.daysLate && invoice.daysLate >= 15) ||
      ((invoice.remindersSent ?? 0) >= 3 && !(invoice.responseReceived ?? false)) ||
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredInvoices.map((invoice) => (
                <tr key={invoice.id} className={`hover:bg-gray-50 ${selectedInvoice === invoice.id ? "bg-blue-50" : ""}`} onClick={() => setSelectedInvoice(invoice.id)}>
                  <td className="py-3 px-4">
                    <div className="font-medium text-gray-900">{invoice.customer}</div>
                    <div className="text-xs text-gray-500">#{invoice.id}</div>
                  </td>
                  <td className="py-3 px-4 text-center font-medium text-gray-900">{formatCurrency(invoice.amount)}</td>
                  <td className="py-3 px-4 text-center text-gray-700">{invoice.dueDate}</td>
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className="text-xs text-gray-700">
                      {(invoice.remindersSent ?? 0) === 0
                        ? "No reminders sent"
                        : `${invoice.remindersSent} reminder${(invoice.remindersSent ?? 0) > 1 ? "s" : ""} sent${invoice.responseReceived ? ", response received" : ", no response"}`}
                    </div>
                  </td>
      ...
      {selectedInvoice && data.invoices.find((inv) => inv.id === selectedInvoice)?.status === "collections" && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          ...
          <div className="flex space-x-3">
            ...
            <button className="flex-1 py-2 px-4 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors flex items-center justify-center">⏸️ Pause Automation</button>
          </div>
        </div>
      )}
    </div>
