      ...
      {keywords.map((keyword, index) => (
        <div key={index} className="py-3 border-b border-gray-100 last:border-b-0">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center">
              <span className={`inline-block px-2 py-1 rounded text-xs font-medium mr-3 ${
                SENTIMENT_COLORS[keyword.sentiment].bg
              } ${SENTIMENT_COLORS[keyword.sentiment].text}`}>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2 relative overflow-hidden">
            <div 
              className={`h-2 rounded-full ${SENTIMENT_COLORS[keyword.sentiment].bar} bg-opacity-50`}
