...
import type { Branch, FinancialData, FinancialInputData, FinancialsFilters, PeriodData } from "@/types/financials";

export class FinancialsMockApi {
  async getFinancialData(_filters: FinancialsFilters): Promise<FinancialData> {
    // Simulate API delay
    ...
    const inputData: FinancialInputData = {
      ...
      branchData: mockBranches.map((branch) => ({
        branchId: branch.id,
        periods: Array.from({ length: 4 }, (_, i) => generatePeriodData(i, branch.id === "1" ? 1 : branch.id === "2" ? 0.7 : 0.5)),
      })),
      consolidatedData: {
