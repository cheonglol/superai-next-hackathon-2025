...
import { Activity, TrendingUp, TrendingDown, AlertTriangle, CheckCircle, DollarSign, Calendar, Target, Zap, BarChart3, ArrowRight, RefreshCw, Percent } from "lucide-react";
...
const CashFlowDiagnosticianPage: React.FC = () => {
  ...
  const cashFlowMetrics = [
    // Operating Cash Flow
    {
      ...
      value: `${formatCurrency(calculatedMetrics.netOperatingCashFlow)}`,
      category: "operating",
      description: "Cash generated from business operations.",
    },
    {
      name: "Operating Cash Flow Margin (%)",
      value: `${calculatedMetrics.operatingCashFlowMargin.toFixed(1)}%`,
      category: "operating",
      description: "% of sales that turns into cash.",
    },
    {
      name: "Cash Conversion Cycle (CCC)",
      value: `${Math.round(calculatedMetrics.cashConversionCycle)} days`,
      category: "operating",
      description: "How long cash is tied up in operations.",
    },
    ...
    { name: "Days Sales Outstanding (DSO)", value: `${Math.round(calculatedMetrics.dso)} days`, category: "working_capital", description: "How fast customers pay you." },
    {
      ...
      value: `${Math.round(calculatedMetrics.dio)} days`,
      category: "working_capital",
      description: "How long stock is sitting before it's sold.",
    },
    {
      name: "Days Payable Outstanding (DPO)",
      value: `${Math.round(calculatedMetrics.dpo)} days`,
      category: "working_capital",
      description: "How long you're taking to pay suppliers.",
    },
    {
      name: "Working Capital Ratio",
      value: `${calculatedMetrics.workingCapitalRatio.toFixed(2)}`,
      category: "working_capital",
      description: "Short-term liquidity (1.2–2.0 is generally healthy).",
    },

    // Liquidity & Buffer
    {
      name: "Cash Reserve Ratio",
      value: `${calculatedMetrics.cashReserveRatio.toFixed(1)} months`,
      category: "liquidity",
      description: "Number of months you can survive without revenue.",
    },
    ...
    // Revenue & Cost Impact
    {
      name: "Gross Profit to Cash Conversion",
      value: `${calculatedMetrics.grossProfitToCashConversion.toFixed(2)}`,
      category: "impact",
      description: "How well gross profit turns into real cash.",
    },
    ...
    {
      name: "Cost of Sales to Cash Outflow",
      value: `${calculatedMetrics.costOfSalesToCashOutflow.toFixed(2)}`,
      category: "impact",
      description: "Tracks overpayment or mismatch between cost and cash.",
    },
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              <div className="flex items-center">
                ...
                <input type="date" value={fyEndingDate} onChange={(e) => setFyEndingDate(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm" />
              </div>
              <button onClick={handleFYEndingDateChange} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center">
                <RefreshCw className="w-4 h-4 mr-2" />
                Analyze
              </button>
            </div>
          </div>
          {lastUpdate && <div className="mt-4 text-xs text-gray-500">Last updated: {lastUpdate}</div>}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === "metrics" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === "leakage" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === "actions" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
            >
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    <p className="text-sm text-green-700">
                      Small, incremental changes that are easy to implement but compound over time. Each 1% or 1-day improvement may seem minor on its own, but together, they
                      create a powerful ripple effect—freeing up cash, lifting profits, and strengthening your business's financial foundation.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    <tr className="bg-gray-100">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Your Current Position</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900"></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-red-600">-193,000</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">701,300</td>
                    </tr>
                    ...
                    <tr className="bg-gray-50">
                      <td colSpan={2} className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"></td>
                    ...
                    {/* Margin Movers rows */}
                    <tr>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Price Increase</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 %</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">51,690</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">66,120</td>
                    </tr>
                    <tr className="bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Volume Increase</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 %</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-red-600">-4,855</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">19,175</td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">COGS Reduction</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 %</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">56,545</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">46,945</td>
                    </tr>
                    <tr className="bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Operating Expenses Reduction</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 %</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">12,162</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">12,162</td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Accounts Receivable Days Reduction</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 day</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">18,115</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900"></td>
                    </tr>
                    <tr className="bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Inventory Days Reduction</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 day</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">12,862</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900"></td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Accounts Payable Days Increase</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900">1 day</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">12,862</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900"></td>
                    </tr>
                    ...
                    <tr className="bg-green-50 font-bold">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Margin Movers Impact</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900"></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-green-700">159,381</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-green-700">144,402</td>
                    </tr>
                    ...
                    <tr className="bg-gray-100">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Your Adjusted Position</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900"></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-red-600">-33,619</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-green-600">845,702</td>
                    </tr>
                  </tbody>
                </table>
              </div>
                  ...
                  <div key={index} className={`border rounded-lg p-4 ${getPriorityColor(action.priority)}`}>
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3">
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-sm">
                          Timeframe: <strong>{action.timeframe}</strong>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs">
                        Difficulty: <span className={getDifficultyColor(action.difficulty)}>{action.difficulty.charAt(0).toUpperCase() + action.difficulty.slice(1)}</span>
                      </span>
